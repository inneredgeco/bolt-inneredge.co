...
interface BlogNewsletterSignupRequest {
  firstName: string;
  email: string;
}
...
Deno.serve(async (req: Request) => {
  ...
  try {
    const { firstName, email }: BlogNewsletterSignupRequest = await req.json();
    ...
    const flodeskSegmentId = Deno.env.get("FLODESK_BLOG_SEGMENT_ID") || "68f80ada11436db6d8879f73";
    ...
    try {
      ...
      console.log("Adding to Flodesk - Step 2: Add to Blog Segment");
      ...
      if (!segmentResponse.ok) {
        ...
        console.error("Failed to add subscriber to Blog segment:", errorText);
        return new Response(
          JSON.stringify({
            success: false,
            error: "Failed to subscribe. Please try again.",
      ...
      console.log("Successfully added to Blog Flodesk segment");
    ...
    console.error("Error processing blog newsletter signup:", error);
